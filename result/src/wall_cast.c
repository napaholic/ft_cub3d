...
void	set_texture_data(t_wall_data *wall_data, t_info *info)
{
	...
	wall_data->tex_pos = (wall_data->draw_start - info->win_hei
			/ 2 + wall_data->lineheight / 2) * wall_data->step_val;
}
...
int	set_color(t_wall_data *wall_data, t_info *info)
{
	...
	tex_y = (int)wall_data->tex_pos & (TEX_HEIGHT - 1);
	...
	if (wall_data->side && wall_data->step_y == POSITIVE)
		...
		texnum = TEX_WALL_W;
	(void)info;
	color = info->texture[texnum][TEX_HEIGHT * tex_y + wall_data->tex_x];
	if (wall_data->side == 1)
