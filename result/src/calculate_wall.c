...
void	init_dda_cast(t_wall_data *wall_data, t_info *info)
{
	...
	{
		wall_data->step_x = -1;
		wall_data->side_dist_x = (info->pos->pos_x - \
			wall_data->map_pos_x) * wall_data->d_dist_x;
	}
	else
	{
		wall_data->step_x = 1;
		wall_data->side_dist_x = (wall_data->map_pos_x + 1.0 - \
			info->pos->pos_x) * wall_data->d_dist_x;
	}
	if (wall_data->raydir_y < 0)
	{
		wall_data->step_y = -1;
		wall_data->side_dist_y = (info->pos->pos_y - \
			wall_data->map_pos_y) * wall_data->d_dist_y;
	}
	else
	{
		wall_data->step_y = 1;
		wall_data->side_dist_y = (wall_data->map_pos_y + 1.0 - \
			info->pos->pos_y) * wall_data->d_dist_y;
	}
}
...
void	set_dda(t_wall_data *wall_data, t_info *info, int count)
{
	...
	wall_data->raydir_x = info->pos->dir_x + info->pos->plane_x * \
		wall_data->camera_x;
	wall_data->raydir_y = info->pos->dir_y + info->pos->plane_y * \
		wall_data->camera_x;
